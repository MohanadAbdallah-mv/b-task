...
class _BackCameraScreenState extends State<BackCameraScreen> {
  ...
  Future<File?> scanDocument() async {
    try {
      ...
      Provider.of<UserProvider>(context, listen: false).setBackIDPath(path);
      return File(path);
    } catch (e) {
      log("error:$e");
      return null;
  ...
  void initState() {
    super.initState();
    String? path = Provider.of<UserProvider>(context, listen: false).backIDPath;
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      log("checked");
      if (path != null) {
        _scannedDocuments = File(path);
        log("_scannedDocuments at init state , back");
